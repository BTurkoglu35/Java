...
public class Fp05 {
    public static void main(String[] args) {
        ...
        System.out.println(ortalamaPuanlarinVerilenSayidanBuyukMu(coursesList, 96));

        System.out.println(kursAdiKelimeyiIceriyorMu(coursesList,"Night"));
        ortalamasiEnYuksekKursunAdiniYazdir(coursesList);
        System.out.println(ortalamaPuanaGoreSiralaVeIlkVerileneleriAtla(coursesList,1));


    }
    //1)Tüm ortalama puanların verilen sayıdan büyük olup olmadığını kontrol etmek için bir method oluşturun.
    public static boolean ortalamaPuanlarinVerilenSayidanBuyukMu(List<Courses> list,double x ){
    ...
    public static boolean kursAdiKelimeyiIceriyorMu(List<Courses>list,String kelime){
  ...
  String sonuc= list.stream().sorted(Comparator.comparing(Courses::getAverageScore).reversed()).findFirst().get().getCourseName();
    ...
    public  static  List<Courses> ortalamaPuanaGoreSiralaVeIlkVerileneleriAtla(List<Courses> list,int x){
