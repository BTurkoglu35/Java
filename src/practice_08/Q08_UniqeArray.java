package practice_08;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class Q08_UniqeArray {
    /*
     * Verilen array deki tekrar eden sayilari, ilki haric silip, tekrarsiz
     * sayilardan olusan bir array haline getiren bir program yaziniz.
     *
     * INPUT : {1,2,2,3,1,4,2,-2,5,6,8,7,5,6,-2,9,3,10}
     * OUTPUT : [1, 2, 3, 4, -2, 5, 6, 8, 7, 9, 10]
     */
    public static void main(String[] args) {

    int[]arr={1,2,2,3,1,4,2,-2,5,6,8,7,5,6,-2,9,3,10};
       // Arrays.sort(arr);
        List<Integer> tekrarsizsayilar=new ArrayList<Integer>();
        int sayac=0;
        for (int i = 0; i <arr.length ; i++) {
            if (!tekrarsizsayilar.contains(arr[i])) {
                tekrarsizsayilar.add(arr[i]);
            }
        }
        System.out.println(tekrarsizsayilar);
       int[] tekrarsizarr = new int[tekrarsizsayilar.size()];
        for (int i = 0; i <tekrarsizsayilar.size(); i++) {
            tekrarsizarr[i] = tekrarsizsayilar.get(i);

        }
        System.out.println(Arrays.toString(tekrarsizarr));







            //       if (arr[i]==arr[i+1]){
     //           sayac++;
     //       }
     //   int[] tekrarsizarr=new int[arr.length-sayac];
     //       f













    }
}
